...
class TestFileAnalyzer:
    ...
    def test_calculate_tokens_invalid_content(self):
        """测试无效内容的 token 计算（应该返回0而不崩溃）"""
        # 使用一些可能导致 tiktoken 出错的内容
...
'''
        ...
        # 验证实际代码的分析结果
        assert result['line_count'] > 10  # 应该有足够的行数
        assert result['char_count'] > 100  # 应该有足够的字符数
        assert result['word_count'] > 20  # 应该有足够的词数
        assert result['token_count'] > 10  # 应该有足够的 token 数
    ...
    def test_file_analysis_with_minimal_content(self, temp_dir):
        ...
        assert result['line_count'] == 0  # 没有换行符
        assert result['char_count'] == 1
        assert result['word_count'] == 1
        assert result['token_count'] >= 1
        assert result['non_empty_line_count'] == 1  # 有一个非空行